...
Ext.ux.panel.DDTabPanel = Ext.extend(Ext.TabPanel, {
	...
	initTab: function(tab, index){
		...
		/**
		 *trick for screen edit/dashboard edit
		 * TODO replace this implementation
		 */
		Ext.get(id).on("click",function(){
			var tabID=this.id.substring(this.id.indexOf("__")+2);
			var tabDom=document.getElementById("t"+tabID);
			if(tabDom!=null&&tabDom.controller!=null){
				if(ScreenEdit!=null){
					ScreenEdit.clickOnTab=true;
					ComponentController.instance.clickHandler(tabDom);
				}
			}
		});
		//END trick
...
Ext.ux.panel.DDTabPanel.DropTarget = Ext.extend(Ext.dd.DropTarget, {
	...
	notifyDrop: function(dd, e, data){
		...
		dd.proxy.hide();
		var origIndex=data.index;
		var fromID=dd.dropEl.id;
		var newIndex=i;
		if(newIndex>origIndex){
			newIndex--;
		}
		if(origIndex==newIndex){//same tab
			return true;
		}
		if(this.tabpanel.hadlerDrop!=null){
			this.tabpanel.hadlerDrop.call(this,fromID,newIndex);
		}
		return true;

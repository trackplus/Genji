...
Ext.define("js.ext.com.track.datasource.FaqDatasource",{
	...
	initComponent: function() {
		...
		if (this.fromIssueNavigator) {
			...
			this.add(CWHF.createHiddenField("fromIssueNavigator", {itemId:"fromIssueNavigator",value:this.fromIssueNavigator}));
		}
		if (this.workItemID) {
			datasourceDisabled = true;
			this.add(CWHF.createHiddenField("workItemID", {itemId:"workItemID",value:this.workItemID}));
		}
		//hidden fields for reports called from project/release specific dashboard
		if (this.dashboardID) {
			this.add(CWHF.createHiddenField("dashboardID", {itemId:"dashboardID",value:this.dashboardID}));
		}
		if (this.dashboardProjectReleaseID) {
			this.add(CWHF.createHiddenField("projectID", {itemId:"projectID",value:this.dashboardProjectReleaseID}));
			datasourceDisabled = true;
		}
		//datasource type
		...
		this.datasourceRadioGroup = CWHF.getRadioGroup("common.datasource", this.width, datasourceList,
			{itemId:"datasourceType",

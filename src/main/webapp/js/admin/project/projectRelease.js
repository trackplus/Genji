...
com.trackplus.admin.project.Release=Ext.define('com.trackplus.admin.project.Release',{
	...
	statics:{
		createEditPanelItems:function(lw){
			return [CWHF.createTextField('common.lbl.name',
				...
				CWHF.createCombo('admin.customize.localeEditor.type.releaseStatus',
					'releaseDetailTO.releaseStatusID', {labelWidth:lw}, null, 'statusID'),
		...
		postDataLoadCombo: function(data, panel) {
			var statusCombo = panel.getComponent('statusID');
			if (statusCombo!=null) {
	...
	initLocalizedLabels: function(projectID) {
		Ext.Ajax.request({
			...
			url: this.baseAction + '!localizedLabels.action',
			scope: this,
	...
	getGridExtraParams: function(node, opts) {
		...
		if (opts!=null && opts['showClosedReleases']!=null) {
	...
	/*private*/getAddParams: function(addAsChild) {
		var addParams = {add:true, addAsChild:addAsChild/*, projectID:this.projectID*/};
		var addToNode = null;
	...
	getTreeContextMenuActions: function(selectedRecord, selectionIsSimple) {
		if (selectedRecord!=null) {
			...
			if (selectedRecord.parentNode!=null && !selectedRecord.parentNode.isRoot()) {
	...
	getGridColumns: function(node) {
			return [{header: getText('common.lbl.name'),
				flex:1, dataIndex: 'label', sortable: false, renderer:this.renderer},
			{header: getText('admin.project.release.lbl.status'),
				flex:1, dataIndex: 'statusLabel', sortable: false, renderer:this.renderer},
			{header: getText('admin.project.release.lbl.dueDate'),
				flex:1, dataIndex: 'dueDate', sortable: false,
				renderer:this.dateRenderer},
			{xtype: 'checkcolumn', header: getText('admin.project.release.lbl.defaultReleaseNoticed'),
				flex:1, dataIndex: 'defaultNoticed', sortable: false},
			{xtype: 'checkcolumn', header: getText('admin.project.release.lbl.defaultReleaseScheduled'),
	...
	canDragDropNode: function(nodeToDrag, copy, overModel) {
		return nodeToDrag!=null && !nodeToDrag.isAncestor(overModel);
	},
	...
	/*getRecommendedReplaceItems: function(result) {
		return [{xtype : 'label',
			html: result.errorMessage},
	        CWHF.createSingleTreePicker("admin.project.release.moveReleaseItems.replacementRelease",
	        "replacementID", result.replacementTree, null,
	        {allowBlank:true,
	         labelWidth:200,
	         margin:'5 0 0 0'
	...
	getToolbarActionChangesForTreeNodeSelect: function(selectedNode) {
		this.actionAddChildRelease.setDisabled(selectedNode==null || this.rootID==selectedNode.data['id']);

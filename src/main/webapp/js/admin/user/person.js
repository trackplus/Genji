...
Ext.define("com.trackplus.admin.user.Person", {
	...
	constructor: function(config) {
		...
		this.profile=Ext.create("com.trackplus.admin.user.Profile",{context:config.context});
		this.profile.isUser = this.isUser;
		config["enableColumnHide"] = true;
		config["enableColumnMove"] = true;
		config["features"] = this.getGridFeatures(config.userLevels);
	...
	init:function(){
		this.initActions();
		Ext.Ajax.request({
			url: this.baseAction + "!getLdapIsOn.action",
			scope: this,
	...
	getGridFields: function() {
		...
		if (this.isUser && this.featureList!=null) {
			Ext.Array.forEach(this.featureList, function(feature) {
	...
	onActivate: function() {
		Ext.Ajax.request({
			url: this.baseAction + "!activate.action",
	...
	onDeactivate: function() {
		Ext.Ajax.request({
			url: this.baseAction + "!deactivate.action",
			params: {selectedPersonIDs: this.getSelectedPersonIDs()},
			scope: this,
			success: function(response) {
				var result = Ext.decode(response.responseText);
	...
	onUserLevel: function(userLevel) {
		var submitUrl = this.baseAction + "!userLevel.action";
	...
	getUserLevelItems: function() {
		...
		var data = null;
		if (sysAdmin) {
			data = this.userLevels;
		} else {
			//a system manager might not set new system admins
			data = [];
			Ext.Array.forEach(this.userLevels, function(record) {
					var userLevelID = record["id"];
					if (userLevelID!=null && userLevelID!=2) {
						data.push(record);
					}
					},
					this);
	...
	onCokpitAssignment: function() {
		var me=this;
		com.trackplus.dashboard.resetDashboard(this.baseAction + "!cokpitAssignment.action",
	...
	getRoleAssignmentGrid: function(personID) {
		var gridComponent = Ext.create("com.trackplus.admin.user.UserRolesInProject",
				{urlStore:"rolesInProjects.action", personID:personID, group:false});
	...
	getFilterAssignmentGrid: function(personIDs) {
	    var gridComponent = Ext.create("com.trackplus.admin.user.FiltersInUserMenus",
	        {urlStore:"filtersInMenu.action", personIDs:personIDs});
	    var grid = gridComponent.getGrid();
	    return [grid];
	},
	...
	onSyncLdap: function() {
		Ext.Ajax.request({
			url: this.baseAction + "!syncldap.action",
			scope: this,
			success: function(response) {
				var result = Ext.decode(response.responseText);
				if (result.success) {
	...
	getColumnModel:function() {
		var columnModel = [{text: getText("admin.user.profile.lbl.userName"),
				...
				filterable: true, renderer:this.dateRenderer
			}, {
	...
	onCheckChange: function(checkBox, rowIndex, checked, eOpts) {
		...
		if (record!=null && feature!=null) {
			...
			Ext.Ajax.request({
				url: this.baseAction + "!changeFeature.action",
				params: params,
				scope: this,
				success: function(response) {
					var result = Ext.decode(response.responseText);
	...
	getSaveParams: function(recordData, operation){
		return {isUser:this.isUser, context:this.getContext(recordData, operation),
			personId: recordData['id']};
	},
	...
	getLoadGridUrl:function() {
		var param = "?isUser=" + this.isUser;
		return this.urlStore + param;
	}
});

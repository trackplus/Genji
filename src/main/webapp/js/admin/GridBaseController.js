...
Ext.define("com.trackplus.admin.GridBaseController",{
	...
	//baseAction:null,
	//the name of the request parameter the selected record's ID will be sent to the server (by edit/delete) 
	//entityID:null,
	...
	onAdd: function(button, event) {
		//var recordData=new Object();
	 	//this.openEntityDialog.call(this, this.getView().getActionTooltip(this.getView().getAddTitleKey()), recordData,"add");
		this.onAddEdit(this.getView().getActionTooltip(this.getView().getAddTitleKey()), null, "add");
	},
	...
	onEdit: function(button, event) {
		...
		this.openEntityDialog.call(this, this.getView().getActionTooltip(this.getView().getEditTitleKey()), recordData,"edit");*/
		var record = this.getView().getSingleSelectedRecord();
		if (record) {
			this.onAddEdit(this.getView().getActionTooltip(this.getView().getAddTitleKey()), record, "edit");
		}
	},
	...
	onCopy: function(button, event) {
		...
		if(CWHF.isNull(recordData)){
	 	...
	 	this.openEntityDialog.call(this, this.getView().getActionTooltip(this.getView().getCopyTitleKey()), recordData,"copy");*/
		var record = this.getView().getSingleSelectedRecord();
		if (record) {
			this.onAddEdit(this.getView().getActionTooltip(this.getView().getAddTitleKey()), record, "copy");
		}
	},
	...
	/*openEntityDialog: function(title, recordData, operation) {
		var me=this;
	...
	onAddEdit: function(title, record, operation) {
	...
	reload:function(refreshParametersObject) {
		this.reloadGrid(this.getView(), refreshParametersObject);
	},
	 ...
	 * Typically same as getStoreExtraParams();
	 */
	/*protected abstract*/getReloadParamsAfterSave:function() {
		return this.getStoreExtraParams();
	},
	/**

...
public class ReportBeanExpandContext {
	...
	public static ReportBeanExpandContext getExpandContext(Map session/*, TPersonBean personBean, QueryContext queryContext*/) {
		...
		if (personBean.isEnableQueryLayout()) {
			filterType = queryContext.getQueryType();
			filterID = queryContext.getQueryID();
		}
		List<GroupFieldTO> groupFields = new LinkedList<GroupFieldTO>();
		SortFieldTO sortFieldTO = LayoutGroupsBL.loadGroupFields(personBean.getObjectID(), null, filterType, filterID, false, groupFields);

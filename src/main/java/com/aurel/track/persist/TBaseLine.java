...
public  class TBaseLine 
    ...
    public boolean hasChanged(TBaseLine oldObject) {
		...
		/*float blEffort = (float) 0.0;
		if (oldObject != null && oldObject.getEffort() != null) {
		    blEffort = oldObject.getEffort().floatValue();
		}
		float thisEffort = (float) 0.0;
		if (this.getEffort() != null) {
		    thisEffort = this.getEffort().floatValue();
		}
	    if ((Math.abs(thisEffort -  blEffort)) > 0.00000001) {
	        result = true;
	    }*/

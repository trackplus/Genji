...
/**
 * Apply field change for custom select fields
 * Differs from the system select by the fact that the value is Integer[] instead of Integer 
 * @author Tamas
 *
 */
public class CustomMultipleSelectFieldChangeApply extends SelectFieldChangeApply {
	private static final Logger LOGGER = LogManager.getLogger(CustomMultipleSelectFieldChangeApply.class);
	
	public CustomMultipleSelectFieldChangeApply(Integer fieldID) {
		super(fieldID, true);
	}

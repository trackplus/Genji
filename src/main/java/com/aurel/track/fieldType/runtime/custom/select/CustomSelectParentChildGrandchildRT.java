...
public class CustomSelectParentChildGrandchildRT extends CustomCompositeBaseRT {
	...
	public ICustomFieldTypeRT getCustomFieldType(int index) {
		List<Integer> hierarchyBeansList;
		switch(index) {
		case 1:
		...
		case 2:
			...
			hierarchyBeansList = new ArrayList<Integer>();
			hierarchyBeansList.add(Integer.valueOf(1));
			return new CustomDependentSelectRT(Integer.valueOf(1), Integer.valueOf(1), hierarchyBeansList);
		case 3:
			//the parent is the second part of the composite 
			//and the child list is the first child list of the parent 
			hierarchyBeansList = new ArrayList<Integer>();
			hierarchyBeansList.add(Integer.valueOf(1));
			hierarchyBeansList.add(Integer.valueOf(1));
			return new CustomDependentSelectRT(Integer.valueOf(2), Integer.valueOf(1), hierarchyBeansList);
		}
		return null;
	}
	...
	public Map getValidators(Integer fieldID, 
		...
		//otherwise the validation message is tripled
	...
	public void processLoadDataSource(SelectContext selectContext, DropDownContainer dropDownContainer) {
		...
		CustomDependentSelectRT grandchildSelect=(CustomDependentSelectRT)getCustomFieldType(3);
		...
		if (parameterCode==null) {
			...
			Object origChildValue=selectContext.getWorkItemBean().getAttribute(selectContext.getFieldID(),parameterCode);
			Integer[] childOptions = CustomSelectUtil.getSelectedOptions(origChildValue);
			if (childOptions==null || childOptions.length==0) {
				//by create: if no default value was set for this list (origParentValue is null or Integer[0] 
				//			after calling the setDefaultAttribute()) then get the first value from the list: 
				///			we should be sure that even when no default value is defined the first entry is 
				//			implicitely selected to have a parentID for getting the grandchild list
				//by edit: if no value (i.e. null) was previousely saved but now the parent list has entries then get the first
				//if print mode, then leave the value empty
				if (!selectContext.isView() && fieldConfigBean.isRequiredString()) {
					//get the previousely loaded child list
					List childList=dropDownContainer.getDataSourceList(MergeUtil.mergeKey(selectContext.getFieldID(), parameterCode));
					if(childList!=null && !childList.isEmpty()){
						//the child list has at least one element set the first one for the grandchildren
						selectContext.getWorkItemBean().setAttribute(selectContext.getFieldID(), parameterCode, 
								new Integer[] {((TOptionBean)childList.get(0)).getObjectID()});
					}
				}
			}
			//grandchild select
			...
			selectContext.setParameterCode(parameterCode);
			grandchildSelect.processLoadDataSource(selectContext, dropDownContainer);
		} else {
			//refresh took place

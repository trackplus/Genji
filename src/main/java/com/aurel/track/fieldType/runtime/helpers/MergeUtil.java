	...
	public static Integer[] getParts(String mergedString) {
		...
		if (stringKeys!=null && stringKeys[0]!=null) {
			try {
				...
				LOGGER.warn("Getting the first part '" + stringKeys[0] + "' of the merged string '" + mergedString + "' failed with " + e.getMessage(), e);
			}
		}
		try {
			...
			LOGGER.warn("Getting the second part '" + stringKeys[1] + "' of the merged string '" + mergedString + "' failed with " + e.getMessage(), e);
		}
		return intKeys;
	}

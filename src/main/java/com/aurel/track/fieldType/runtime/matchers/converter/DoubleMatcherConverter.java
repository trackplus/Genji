 ...
 */
public class DoubleMatcherConverter implements MatcherConverter{
	private static final Logger LOGGER = LogManager.getLogger(DoubleMatcherConverter.class);
	private static DoubleMatcherConverter instance;
	public static DoubleMatcherConverter getInstance(){
		if(instance==null){
			instance=new DoubleMatcherConverter();
		}
		return instance;
	}
	...
	public Object fromValueString(String valueString, Locale locale, Integer matcherRelation) {
		if (matcherRelation!=null) {
			...
			case MatchRelations.LESS_THAN:
			case MatchRelations.LESS_THAN_EQUAL:
				return DoubleNumberFormatUtil.getInstance().parseGUI(valueString, locale);
			}
		}
		return null;
	}

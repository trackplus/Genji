...
/**
 * Bulk setter for integer fields
 * @author Tamas
 *
 */
public class IntegerBulkSetter extends AbstractBulkSetter {
	private static final Logger LOGGER = LogManager.getLogger(IntegerBulkSetter.class);
	public IntegerBulkSetter(Integer fieldID) {
		super(fieldID);
	}
	...
	public ErrorData setWorkItemAttribute(TWorkItemBean workItemBean,
		...
		switch (getRelation()) {
		case BulkRelations.SET_TO:
			workItemBean.setAttribute(fieldID, parameterCode, intValue);
			break;
		case BulkRelations.ADD_IF_SET:
			if (intValue!=null) {
				Integer originalValue = (Integer)workItemBean.getAttribute(fieldID, parameterCode);
				if (originalValue!=null) {
					int newValue = originalValue.intValue() + intValue.intValue();
					workItemBean.setAttribute(fieldID, parameterCode, Integer.valueOf(newValue));
				}
			}
			break;
		case BulkRelations.ADD_OR_SET:

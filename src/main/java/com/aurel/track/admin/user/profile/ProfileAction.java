...
public class ProfileAction extends ActionSupport implements Preparable, SessionAware, ServletResponseAware {
	...
	public String save (){
		...
		//If Preferred Language: browser => we need to set the locale to an empty string.
		//Track with Ext JS 4.x if the model contains empty string as id then submitted id will be empty string
		//Track with Ext JS 6.x if the model contains empty string as id then submitted id will be generated by framework.
		//For this reason here must be set the locale to an empty string instead of the dummy browser id. (Only for this item).
		if(mainTo.getLocale() != null && mainTo.getLocale().equals("browser")) {
			mainTo.setLocale("");
		}

 ...
 * @author adib
 */
public class ItemNavigatorAction extends ActionSupport implements Preparable, SessionAware, ServletRequestAware, ServletResponseAware, ApplicationAware {
	...
	protected String prepareInitData(QueryContext queryContext, ApplicationBean appBean, Navigator nav) {
		return ItemNavigatorBL.prepareInitData(nav, appBean, personBean, locale, queryContext, session, lite, workItemID, actionID, forceAllItems,
				isUseIssueTypeIcon());
	}
	...
	public String navigate() {
		...
		try {
			...
			sb.append("{");
			JSONUtility.appendJSONValue(sb, "issues", ReportItemJSON.encodeReportBeans(reportBeans, layoutTO, includeLongFields, locale, useProjectSpecificID,
					personBean, queryFieldCSS, plainData, exclusiveShortFields, includePercentDone, isUseIssueTypeIcon()));
	...
	protected String executeInternal(QueryContext queryContext, boolean includeLayout, ReportBeanExpandContext reportBeanExpandContext,
			boolean forcePersonLayout) {
		if (queryContext == null) {
			...
			try {
				...
				if (fromSession == true) {
					...
					if (gantt) {
						...
						reportBeansDB = TreeFilterExecuterFacade.getInstantTreeFilterReportBeanListNoReport(filterUpperTO, qNode, null, personBean, locale,
								null, null);
					}
				} else {
		...
		if (plugin != null) {
			reportBeanExpandContext = plugin.updateReportBeanExpandContext(personBean, reportBeanList, reportBeanExpandContext, session, queryContext,
					forcePersonLayout);
		}
		...
		if (!forceAllItems && count > ItemNavigatorBL.MAX_ISSUE_NO) {
			overflowItems = count;
		...
		if (queryRepositoryBean != null) {
			String lviewID = NavigatorLayoutBL.getSavedItemFilterView(queryContext.getQueryID(), queryContext.getQueryType());
			isFilterView = lviewID != null && !"".equals(lviewID);
			if (isFilterView) {
		...
		try {
			...
			/*
			 * Gantt HOLIDAYS config The holidays array contains the free days
			 * dates. This code block will be used when Genji calendar is
			 * implemented. This is sample for useing.
			 *
			 */
	...
	public String getAllProjectHierarchy() {
		JSONUtility.encodeJSON(servletResponse, JSONUtility
				.getTreeHierarchyJSON(ReleaseBL.getReleaseNodesEager(personBean, null, true, true, true, true, null, false, true, false, null, locale)));
		return null;
	}
	...
	public String getLastSelectedForTablet() {
		...
		ApplicationBean appBean = (ApplicationBean) application.get(Constants.APPLICATION_BEAN);
		initData = ItemNavigatorBL.prepareInitData(appBean, personBean, locale, queryContext, session, lite, workItemID, actionID, forceAllItems,
				isUseIssueTypeIcon());
		JSONUtility.encodeJSON(servletResponse, initData);
		return null;
	}

	// This method returns a JSON object for tablet client with available
	// filters: label, id
	...
	// This method returnes a JSON object for tablet client with available
	// status list. (used in accordion)
	...
	public String loadComboDataByFieldID() {
		...
		if (fieldID != null) {
			...
			// If there is no item selected in combo box then must be set to
			// null, otherwise will produce JSON error.
	...
	public String indent() {
		...
		workItemID = FieldsManagerRT.saveOneField(personBean.getObjectID(), workItemID, locale, false, SystemFields.INTEGER_SUPERIORWORKITEM, targetWorkItemID,
				true, null, errorList);
	...
	public String outdent() {
		...
		try {
			...
			LOGGER.warn("Loading the workItemBean by outdent for key " + workItemID + " failed with " + e.getMessage());
		}
		...
		if (workItemBean != null) {
			...
			if (parentID != null) {
				...
				try {
					...
					LOGGER.warn("Loading the parent workItemBean by outdent for key " + parentID + " failed with " + e.getMessage());
				}
				...
				workItemID = FieldsManagerRT.saveOneField(personBean.getObjectID(), workItemID, locale, false, SystemFields.INTEGER_SUPERIORWORKITEM,
						grandParent, true, contextInformation, errorList);
			}
		}
		if (!errorList.isEmpty()) {
	...
	public String changeOneField() {
		...
		// prepare the values for specific (project dependent) lists as expected
		// by the massOperation
	...
	public String removeParent() {
		if (workItemID != null) {
			List<ErrorData> errorList = new ArrayList<ErrorData>();
			FieldsManagerRT.saveOneField(personBean.getObjectID(), workItemID, locale, false, SystemFields.INTEGER_SUPERIORWORKITEM, null, false, null,
					errorList);
		}
		try {
			JSONUtility.prepareServletResponseJSON(ServletActionContext.getResponse());
	...
	public void setRemoveViolatedLinksAndCascade(boolean removeViolatedLinksAndCascade) {

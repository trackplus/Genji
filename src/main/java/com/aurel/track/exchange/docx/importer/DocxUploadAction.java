...
public class DocxUploadAction extends ActionSupport implements Preparable, SessionAware, ServletResponseAware {
	...
	public String upload() {
		...
		/*workItemID=275;
		try {
			TAttachmentBean attachmentBean = AttachBL.save(workItemID, null, uploadFileFileName, inputStream, personID);
			if (attachmentBean!=null) {
				attachmentID = attachmentBean.getObjectID();
			}
		} catch (AttachBLException e) {
			LOGGER.error("Uploading the docx file failed with " + e.getMessage(), e);
		}*/	
		//return DOCX_IMPORT;
	}
